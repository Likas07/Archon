...
| Declarative child checkout identity (`workflow.start_commit`) | ✅ admitted (data-only) | The engine must see the immutable full commit identity to create the requested worktree before the child starts; it is valid only with explicit `isolation: worktree`, and accepts a literal lowercase 40-hex SHA or a validated upstream output reference. It coordinates checkout provenance without computing structure, so a script node cannot replace the governance seam |
